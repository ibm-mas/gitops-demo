...
>  This achieved through via the use of the Helm `range` control structure in the template. This approach was used in favour of ArgoCD [Application Sets](https://argo-cd.readthedocs.io/en/stable/operator-manual/applicationset/) in these cases due to several limitations with Application Sets. For instance, Applications generated by Application Sets do not participate properly in the [ArgoCD syncwave](https://argo-cd.readthedocs.io/en/stable/user-guide/sync-waves/) of the parent Application, meaning that we would have no way of ensuring that resources would be configured in the correct order.
